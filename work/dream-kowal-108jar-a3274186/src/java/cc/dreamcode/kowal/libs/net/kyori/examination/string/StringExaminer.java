...
public class StringExaminer extends AbstractExaminer<String>
{
    ...
    protected <E> String array(final E[] array, @NotNull final Stream<String> elements) {
        return (String)elements.collect((Collector)StringExaminer.COMMA_SQUARE);
    }
    ...
    protected String examinable(@NotNull final String name, @NotNull final Stream<Map.Entry<String, String>> properties) {
        return name + (String)properties.map(property -> (String)property.getKey() + '=' + (String)property.getValue()).collect((Collector)StringExaminer.COMMA_CURLY);
    }
    
    @NotNull
    @Override
    protected <K, V> String map(@NotNull final Map<K, V> map, @NotNull final Stream<Map.Entry<String, String>> entries) {
        return (String)entries.map(entry -> (String)entry.getKey() + '=' + (String)entry.getValue()).collect((Collector)StringExaminer.COMMA_CURLY);
    }
    ...
    public String examine(final char value) {
        return Strings.wrapIn((String)this.escaper.apply((Object)String.valueOf(value)), '\'');
    }
    ...
    protected <T> String stream(@NotNull final Stream<T> stream) {
        return (String)stream.map(this::examine).collect((Collector)StringExaminer.COMMA_SQUARE);
    }
    
    @NotNull
    @Override
    protected String stream(@NotNull final DoubleStream stream) {
        return (String)stream.mapToObj(this::examine).collect((Collector)StringExaminer.COMMA_SQUARE);
    }
    ...
    public String examine(@Nullable final String value) {
        ...
        return Strings.wrapIn((String)this.escaper.apply((Object)value), '\"');
    }
    ...
    static {
        DEFAULT_ESCAPER = (string -> string.replace((CharSequence)"\"", (CharSequence)"\\\"").replace((CharSequence)"\\", (CharSequence)"\\\\").replace((CharSequence)"\b", (CharSequence)"\\b").replace((CharSequence)"\f", (CharSequence)"\\f").replace((CharSequence)"\n", (CharSequence)"\\n").replace((CharSequence)"\r", (CharSequence)"\\r").replace((CharSequence)"\t", (CharSequence)"\\t"));
        COMMA_CURLY = Collectors.joining((CharSequence)", ", (CharSequence)"{", (CharSequence)"}");
        COMMA_SQUARE = Collectors.joining((CharSequence)", ", (CharSequence)"[", (CharSequence)"]");
    }
