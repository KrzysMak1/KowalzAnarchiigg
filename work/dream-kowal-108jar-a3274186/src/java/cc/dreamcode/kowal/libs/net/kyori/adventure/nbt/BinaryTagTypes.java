...
public final class BinaryTagTypes
{
    ...
    static {
        ...
        BYTE = BinaryTagType.registerNumeric(ByteBinaryTag.class, (byte)1, input -> ByteBinaryTag.byteBinaryTag(input.readByte()), (tag, output) -> output.writeByte((int)tag.value()));
        SHORT = BinaryTagType.registerNumeric(ShortBinaryTag.class, (byte)2, input -> ShortBinaryTag.shortBinaryTag(input.readShort()), (tag, output) -> output.writeShort((int)tag.value()));
        INT = BinaryTagType.registerNumeric(IntBinaryTag.class, (byte)3, input -> IntBinaryTag.intBinaryTag(input.readInt()), (tag, output) -> output.writeInt(tag.value()));
        LONG = BinaryTagType.registerNumeric(LongBinaryTag.class, (byte)4, input -> LongBinaryTag.longBinaryTag(input.readLong()), (tag, output) -> output.writeLong(tag.value()));
        FLOAT = BinaryTagType.registerNumeric(FloatBinaryTag.class, (byte)5, input -> FloatBinaryTag.floatBinaryTag(input.readFloat()), (tag, output) -> output.writeFloat(tag.value()));
        DOUBLE = BinaryTagType.registerNumeric(DoubleBinaryTag.class, (byte)6, input -> DoubleBinaryTag.doubleBinaryTag(input.readDouble()), (tag, output) -> output.writeDouble(tag.value()));
        BYTE_ARRAY = BinaryTagType.register(ByteArrayBinaryTag.class, (byte)7, input -> {
            ...
            try (final BinaryTagScope ignored = TrackingDataInput.enter(input, length)) {
                ...
                ByteArrayBinaryTag.byteArrayBinaryTag(value);
                return;
            }
        }, (tag, output) -> {
        ...
        STRING = BinaryTagType.register(StringBinaryTag.class, (byte)8, input -> StringBinaryTag.stringBinaryTag(input.readUTF()), (tag, output) -> output.writeUTF(tag.value()));
        LIST = BinaryTagType.register(ListBinaryTag.class, (byte)9, input -> {
            ...
            try (final BinaryTagScope ignored2 = TrackingDataInput.enter(input, length2 * 8L)) {
                final ArrayList tags = new ArrayList(length2);
                for (int i = 0; i < length2; ++i) {
                    ((List)tags).add((Object)type.read(input));
                }
                ListBinaryTag.listBinaryTag(type, (List<BinaryTag>)tags);
                return;
            }
        }, (tag, output) -> {
            output.writeByte((int)tag.elementType().id());
            final int size = tag.size();
            output.writeInt(size);
            tag.iterator();
            final Iterator iterator;
            while (iterator.hasNext()) {
                final BinaryTag item = (BinaryTag)iterator.next();
        ...
        COMPOUND = BinaryTagType.register(CompoundBinaryTag.class, (byte)10, input -> {
            try (final BinaryTagScope ignored3 = TrackingDataInput.enter(input)) {
                final HashMap tags2 = new HashMap();
                while (true) {
                    final BinaryTagType<? extends BinaryTag> type2 = BinaryTagType.binaryTagType(input.readByte());
                    final Object o;
                    if (o != BinaryTagTypes.END) {
                        final String key = input.readUTF();
                        final BinaryTag tag2 = (BinaryTag)type2.read(input);
                        ((Map)tags2).put((Object)key, (Object)tag2);
                    }
                    else {
                        break;
                    }
                }
                return new CompoundBinaryTagImpl((Map<String, BinaryTag>)tags2);
            }
        }, (tag, output) -> {
            tag.iterator();
            ...
            while (iterator2.hasNext()) {
                ...
                if (value3 != null) {
                    ...
                    output.writeByte((int)type3.id());
                    if (type3 != BinaryTagTypes.END) {
                        output.writeUTF((String)entry.getKey());
                        BinaryTagType.writeUntyped(type3, value3, output);
                    }
                    else {
                        continue;
                    }
            ...
            output.writeByte((int)BinaryTagTypes.END.id());
            return;
        });
        INT_ARRAY = BinaryTagType.register(IntArrayBinaryTag.class, (byte)11, input -> {
            ...
            try (final BinaryTagScope ignored4 = TrackingDataInput.enter(input, length3 * 4L)) {
                ...
                IntArrayBinaryTag.intArrayBinaryTag(value4);
                return;
            }
        }, (tag, output) -> {
        ...
        LONG_ARRAY = BinaryTagType.register(LongArrayBinaryTag.class, (byte)12, input -> {
            ...
            try (final BinaryTagScope ignored5 = TrackingDataInput.enter(input, length5 * 8L)) {
                ...
                LongArrayBinaryTag.longArrayBinaryTag(value6);
                return;
            }
        }, (tag, output) -> {
