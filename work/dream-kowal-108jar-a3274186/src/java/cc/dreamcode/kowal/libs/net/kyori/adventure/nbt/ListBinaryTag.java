...
public interface ListBinaryTag extends ListTagSetter<ListBinaryTag, BinaryTag>, BinaryTag, Iterable<BinaryTag>
{
    ...
    default <T extends BinaryTag> Builder<T> builder(@NotNull final BinaryTagType<T> type) {
        if (type == BinaryTagTypes.END) {
            throw new IllegalArgumentException("Cannot create a list of " + (Object)BinaryTagTypes.END);
        }
        return new ListTagBuilder<T>(type);
    }
    
    @NotNull
    default ListBinaryTag listBinaryTag(@NotNull final BinaryTagType<? extends BinaryTag> type, @NotNull final List<BinaryTag> tags) {
        if (tags.isEmpty()) {
            return empty();
        }
        if (type == BinaryTagTypes.END) {
        ...
        return new ListBinaryTagImpl(type, (List<BinaryTag>)new ArrayList((Collection)tags));
    }
