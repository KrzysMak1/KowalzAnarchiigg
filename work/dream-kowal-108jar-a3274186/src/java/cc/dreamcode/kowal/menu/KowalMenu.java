    ...
    public BukkitMenu build(@NonNull final HumanEntity humanEntity) {
        ...
        builder.getItems().forEach((slot, item) -> {
            ...
            if (this.pluginConfig.upgradeAcceptSlot == slot) {
                bukkitMenu.setItem((int)slot, ItemBuilder.of(item).fixColors((Map<String, Object>)new MapBuilder<String, Object>().put("level", currentLevel).put("new", currentLevel + 1).put("items", level.getUpgradeItemsLore()).put("cost", level.getCostLore()).put("status", this.canAfford(player, level) ? this.pluginConfig.canUpgradeStatus : this.pluginConfig.cannotUpgradeStatus).build()).toItemStack(), (Consumer<InventoryClickEvent>)(event -> {
