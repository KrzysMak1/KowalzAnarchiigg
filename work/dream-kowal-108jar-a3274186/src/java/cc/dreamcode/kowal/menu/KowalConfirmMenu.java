...
import cc.dreamcode.kowal.libs.cc.dreamcode.menu.adventure.setup.BukkitMenuPlayerSetup;
...
public class KowalConfirmMenu implements BukkitMenuPlayerSetup
{
    ...
    public BukkitMenu build(@NonNull final HumanEntity humanEntity) {
        ...
        if (this.mode.equals((Object)KowalMenuMode.METAL)) {
            bukkitMenu.setItem(this.pluginConfig.confirmAcceptSlot, ItemBuilder.of(this.pluginConfig.confirmModeMetal).fixColors((Map<String, Object>)Collections.singletonMap((Object)"chance", (Object)this.level.getChance())).toItemStack(), (Consumer<InventoryClickEvent>)this::handleClick);
        }
        else {
            bukkitMenu.setItem(this.pluginConfig.confirmAcceptSlot, ItemBuilder.of(this.pluginConfig.confirmModeKamien).fixColors((Map<String, Object>)Collections.singletonMap((Object)"chance", (Object)this.level.getChance())).toItemStack(), (Consumer<InventoryClickEvent>)this::handleClick);
        }
        return bukkitMenu;
    }
    ...
    private void handleClick(final InventoryClickEvent event) {
        ...
        if (newLevel >= 7) {
            ...
            if (randomEffect != null) {
                newItem.withNbt((Plugin)this.plugin, "upgrade-effect", random.getData()).appendLore(randomEffect.getLore()).fixColors((Map<String, Object>)new MapBuilder<String, Object>().put("level", newLevel).put("chance", randomEffect.getAmplifierChance()).build());
            }
        }
        else {
            newItem.fixColors((Map<String, Object>)Collections.singletonMap((Object)"level", (Object)newLevel));
        }
